...
void XSimulation::Initial(std::istream &in) {
  ...
  for (l = 1; l <= N; ++l)
    for (j = 1; j <= P; ++j) {
      ...
      for (i = 0; i < D; ++i)
        particles[index].coor[i] = L / 2 + 0.5 * (XRandFloat()-0.5) + 0.5;
    }
  for (i = 0; i < D; ++i)
    vs[i] /= N*P;
  for (l = 1; l <= N; ++l)
...
void XSimulation::MakeStat() {
  ...
  for (j = 1; j <= P; ++j) {
    ...
    c /= N;
    XNum dis = MinimumImage2(c-L/2);
    /*if (std::abs(dis-0.0) > incre/3) continue;
    dis = std::abs(dis);
    for (l = 1; l <= N; ++l) {
      int i = Index(l, j);
...
void XCreateWorker(int index, int N, int P, XNum omgP, XNum L, XParticle *particles, XNum *ENkCache2, XNum *ENkCache3, XNum **ENkCache, XParticle *particles2, int NN, XNum *VBCache, XNum *ForceVBCache, int NT, XNum vi, XNum beta, XParticle center, XNum omg0, XNum g, XNum s, XNum h, int Nb, XParticle *particlesb, XNum omgz, XNum g2, XNum s2) {
...
void XSimulation::SpamWorkers() {
  ...
  for (i = 0; i < thread_count; ++i) {
    ...
    if (i == thread_count-1)
      N3 = N-N2*i;
    std::thread t(XCreateWorker, i, N3, P, omgP, L, &particles[i*N2*P], &ENkCache2[i*N2], ENkCache2, ENkCache, particles, i*N2, VBCache, &ForceVBCache[D*P*i*N2], N, vi, beta, center, omg0, g, s, h, Nb, &particles[Na*P], omgz, g2, s2);
    t.detach();
  }
  ENkCache = &ENkCache[Na];
  VBCache = &VBCache[Na+1];
  ...
  for (i = 0; i < thread_count; ++i) {
    ...
    if (i == thread_count-1)
      N3 = N-N2*i;
    std::thread t(XCreateWorker, Na+i, N3, P, omgP, L, &particles[i*N2*P], &ENkCache2[i*N2], ENkCache2, ENkCache, particles, i*N2, VBCache, &ForceVBCache[D*P*i*N2], N, vi, beta, center, omg0, g, s, h, Na, &particles[-Na*P], omgz, g2, s2);
    t.detach();
  }
  N = tmpN;
  ENkCache = &ENkCache[-Na];
